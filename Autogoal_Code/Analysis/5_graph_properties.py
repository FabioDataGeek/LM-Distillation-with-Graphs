...
for i in tqdm(range(len(folder)), desc="Extracting properties for each subdataset", colour="green"):
    ...
    for explanation_id, explanation in tqdm(explanations.items(), desc="Extracting properties for each graph"):    
        ...
        for modality in ['graph', 'subgraph', 'difference_graph']:
            ...
            if not os.path.exists(f"/home/coder/autogoal/properties/{dataset}/{prediction}/{modality}/correct"):
                os.makedirs(f"/home/coder/autogoal/properties/{dataset}/{prediction}/{modality}/correct/", exist_ok=True)
            
            if not os.path.exists(f"/home/coder/autogoal/properties/{dataset}/{prediction}/incorrect"):
                os.makedirs(f"/home/coder/autogoal/properties/{dataset}/{prediction}/{modality}/incorrect", exist_ok=True)
...
for label in label_list:
    for modality in ['graph', 'subgraph', 'difference_graph']:
        with open(f"/home/coder/autogoal/properties/{dataset}/{label}/{modality}/correct/properties.pkl", "wb") as f:
            pkl.dump(properties_dict[label]['correct'][modality], f)
            
        with open(f"/home/coder/autogoal/properties/{dataset}/{label}/{modality}/incorrect/properties.pkl", "wb") as f:
