...
def eliminate_cross_links_and_isolated_nodes(G, root=0):
    """
    ...
    for i in range(num_labels): 
        for correctness in [True, False]:
            for type in ['graph', 'subgraph', 'difference_graph']:
                if not os.path.exists(f'/home/coder/autogoal/structural_properties/summaries/'):
                    os.makedirs(f'/home/coder/autogoal/structural_properties/summaries/', exist_ok=True)
                    summaries[(type, i, correctness)].to_csv(f'/home/coder/autogoal/structural_properties/summaries/summary_{real_labels[i]}_{dataset}_{type}_{correctness}.csv')
                else:
                    summaries[(type, i, correctness)].to_csv(f'/home/coder/autogoal/structural_properties/summaries/summary_{real_labels[i]}_{dataset}_{type}_{correctness}.csv')
    ...
    for i in tqdm(range(num_labels), desc='Generating heatmaps, for dataset: ' + dataset): 
        for correctness in [True, False]:
            for type in ['graph', 'subgraph', 'difference_graph']:
                ...
                if not os.path.exists(f"/home/coder/autogoal/structural_properties/heatmaps/"):
                    os.makedirs(f"/home/coder/autogoal/structural_properties/heatmaps/", exist_ok=True)
                plt.savefig(f"/home/coder/autogoal/structural_properties/heatmaps/heatmap_{real_labels[i]}_{type}_{dataset}_{'' if correctness else 'mis'}classified.pdf", 
...
def histograms(subdataframes, dataset, DATASET_LABELS):
    ...
    output_dir = "/home/coder/autogoal/structural_properties/histograms/"
